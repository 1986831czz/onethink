...
//  Magic Round
//
//  Created by Nguyen Dac Phuoc on 4/25/19.
...
class EasyScene: SKScene {
    ...
    var normal0:SKSpriteNode?
    var normal1:SKSpriteNode?
    ...
    override func didMove(to view: SKView) {
        ...
        normal0 = SKSpriteNode(imageNamed: "normal0.png")
        normal0?.position = CGPoint(x: self.size.width/2, y: 50)
        normal0?.name = "normal0"
        normal0?.zPosition = 2
        self.addChild(normal0!)
        
        normal1 = SKSpriteNode(imageNamed: "normal1.png")
        normal1?.position = CGPoint(x: self.size.width/2, y: (normal0?.position.y)! + (normal0?.size.height)!)
        normal1?.name = "normal1"
        normal1?.zPosition = 3
        self.addChild(normal1!)
    ...
    @objc func createCircle() {
        let index:Int = Int(arc4random_uniform(2))
        ...
        circle.name = "normal"+String(index)
        circle.zPosition = 5
        self.addChild(circle)
        arrCircle.add(item_: circle)
        
        let actionMove:SKAction = SKAction.moveTo(y: normal0!.position.y, duration: TimeInterval(1.5))
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        for touch in (touches ) {
            ...
            if(node.name == "buttonMenu") {
                ...
                //                normal0?.run(actionRotation)
                
                let yNormal0 = normal0?.position.y
                let yNormal1 = normal1?.position.y
                normal0?.position.y = yNormal1!
                normal1?.position.y = yNormal0!
            }
        }
    }
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if(isUpdate == true) {
            for i in 0..<arrCircle.size() {
                ...
                if(self.collisionWith(sprite0: normal0!, sprite1: image)) {
                    if(normal0?.name == image.name) {
                        countScore += 1
                        ...
                        UserDefaults.standard.set(countScore, forKey: "currentScoreNormal")
                        let object: Int = UserDefaults.standard.integer(forKey: "highscoreNormal")
                        
                        if(object > 0) {
                            if(object < countScore) {
                                UserDefaults.standard.set(countScore, forKey: "highscoreNormal")
                ...
                else if(self.collisionWith(sprite0: normal1!, sprite1: image)) {
                    if(normal1?.name == image.name) {
                        countScore += 1
                        labelScore?.text = "Score: "+String(countScore)
                        arrCircle.removeAtIndex(index: i)
    ...
    func submitScore() {
        let score = GKScore(leaderboardIdentifier: "net.kingcorp.rotationendless")
        score.value = Int64(UserDefaults.standard.integer(forKey: "highscoreNormal"))
