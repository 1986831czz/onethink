...
#import "CLMustacheTool.h"
#import "CLCircleView.h"

static NSString* const kCLMustacheToolEmoticonPathKey = @"EmoticonPath";

@interface _CLMustacheTool : UIView
+ (void)setActiveEmoticonView:(_CLMustacheTool*)view;
...
{
    return NSLocalizedStringWithDefaultValue(@"CLMustacheTool_DefaultTitle", nil, [CLImageEditorTheme bundle], @"Mustache", @"");
}
...
{
    return [[[CLImageEditorTheme bundle] bundlePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@/Mustache", NSStringFromClass(self)]];
}

+ (NSDictionary*)optionalInfo
{
    return @{kCLMustacheToolEmoticonPathKey:[self defaultEmoticonPath]};
}
...
{
    [_CLMustacheTool setActiveEmoticonView:nil];
...
- (void)setEmoticonMenu
{
    ...
    NSString *EmoticonPath = self.toolInfo.optionalInfo[kCLMustacheToolEmoticonPathKey];
...
- (void)tappedEmoticonPanel:(UITapGestureRecognizer*)sender
{
    ...
    if(filePath)
    {
        _CLMustacheTool *view = [[_CLMustacheTool alloc] initWithImage:[UIImage imageWithContentsOfFile:filePath]];
        ...
        [_CLMustacheTool setActiveEmoticonView:view];
    }
...
@implementation _CLMustacheTool
{
...
+ (void)setActiveEmoticonView:(_CLMustacheTool*)view
{
    static _CLMustacheTool *activeView = nil;
    if(view != activeView)
    {
...
- (id)initWithImage:(UIImage *)image
{
    ...
    if(self){
        ...
        [_deleteButton setImage:[CLImageEditorTheme imageNamed:[CLMustacheTool class] image:@"btn_delete.png"] forState:UIControlStateNormal];
...
{
    _CLMustacheTool *nextTarget = nil;
    
    const NSInteger index = [self.superview.subviews indexOfObject:self];
    
    for(NSInteger i=index+1; i<self.superview.subviews.count; ++i){
        UIView *view = [self.superview.subviews objectAtIndex:i];
        if([view isKindOfClass:[_CLMustacheTool class]]){
            nextTarget = (_CLMustacheTool*)view;
            break;
        }
    }
