...
#import "CLBeardTool.h"
#import "CLCircleView.h"

static NSString* const kCLBeardToolEmoticonPathKey = @"EmoticonPath";

@interface _CLBeardTool : UIView
+ (void)setActiveEmoticonView:(_CLBeardTool*)view;
...
{
    return NSLocalizedStringWithDefaultValue(@"CLBeardTool_DefaultTitle", nil, [CLImageEditorTheme bundle], @"Beard", @"");
}
...
{
    return [[[CLImageEditorTheme bundle] bundlePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@/Beard", NSStringFromClass(self)]];
}

+ (NSDictionary*)optionalInfo
{
    return @{kCLBeardToolEmoticonPathKey:[self defaultEmoticonPath]};
}
...
{
    [_CLBeardTool setActiveEmoticonView:nil];
...
- (void)setEmoticonMenu
{
    ...
    NSString *EmoticonPath = self.toolInfo.optionalInfo[kCLBeardToolEmoticonPathKey];
...
- (void)tappedEmoticonPanel:(UITapGestureRecognizer*)sender
{
    ...
    if(filePath)
    {
        _CLBeardTool *view = [[_CLBeardTool alloc] initWithImage:[UIImage imageWithContentsOfFile:filePath]];
        ...
        [_CLBeardTool setActiveEmoticonView:view];
    }
...
+ (void)setActiveEmoticonView:(_CLBeardTool*)view
{
    static _CLBeardTool *activeView = nil;
    if(view != activeView)
    {
...
- (id)initWithImage:(UIImage *)image
{
    ...
    if(self){
        ...
        [_deleteButton setImage:[CLImageEditorTheme imageNamed:[CLBeardTool class] image:@"btn_delete.png"] forState:UIControlStateNormal];
...
{
    _CLBeardTool *nextTarget = nil;
    
    const NSInteger index = [self.superview.subviews indexOfObject:self];
    
    for(NSInteger i=index+1; i<self.superview.subviews.count; ++i){
        UIView *view = [self.superview.subviews objectAtIndex:i];
        if([view isKindOfClass:[_CLBeardTool class]]){
            nextTarget = (_CLBeardTool*)view;
            break;
        }
    }
