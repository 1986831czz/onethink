...
#import "CLGlassesTool.h"
#import "CLCircleView.h"

static NSString* const kCLGlassesToolEmoticonPathKey = @"EmoticonPath";

@interface _CLGlassesTool : UIView
+ (void)setActiveEmoticonView:(_CLGlassesTool*)view;
...
{
    return NSLocalizedStringWithDefaultValue(@"CLGlassesTool_DefaultTitle", nil, [CLImageEditorTheme bundle], @"Glasses", @"");
}
...
{
    return [[[CLImageEditorTheme bundle] bundlePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@/Glasses", NSStringFromClass(self)]];
}

+ (NSDictionary*)optionalInfo
{
    return @{kCLGlassesToolEmoticonPathKey:[self defaultEmoticonPath]};
}
...
{
    [_CLGlassesTool setActiveEmoticonView:nil];
...
- (void)setEmoticonMenu
{
    ...
    NSString *EmoticonPath = self.toolInfo.optionalInfo[kCLGlassesToolEmoticonPathKey];
...
- (void)tappedEmoticonPanel:(UITapGestureRecognizer*)sender
{
    ...
    if(filePath)
    {
        _CLGlassesTool *view = [[_CLGlassesTool alloc] initWithImage:[UIImage imageWithContentsOfFile:filePath]];
        ...
        [_CLGlassesTool setActiveEmoticonView:view];
    }
...
+ (void)setActiveEmoticonView:(_CLGlassesTool*)view
{
    static _CLGlassesTool *activeView = nil;
    if(view != activeView)
    {
...
- (id)initWithImage:(UIImage *)image
{
    ...
    if(self){
        ...
        [_deleteButton setImage:[CLImageEditorTheme imageNamed:[CLGlassesTool class] image:@"btn_delete.png"] forState:UIControlStateNormal];
...
{
    _CLGlassesTool *nextTarget = nil;
    
    const NSInteger index = [self.superview.subviews indexOfObject:self];
    
    for(NSInteger i=index+1; i<self.superview.subviews.count; ++i){
        UIView *view = [self.superview.subviews objectAtIndex:i];
        if([view isKindOfClass:[_CLGlassesTool class]]){
            nextTarget = (_CLGlassesTool*)view;
            break;
        }
    }
