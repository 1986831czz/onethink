...
#import "CLHairstyleTool.h"
#import "CLCircleView.h"

static NSString* const kCLHairstyleToolEmoticonPathKey = @"EmoticonPath";

@interface _CLHairstyleView : UIView
+ (void)setActiveEmoticonView:(_CLHairstyleView*)view;
...
@implementation CLHairstyleTool
{
...
{
    return NSLocalizedStringWithDefaultValue(@"CLHairstyleTool_DefaultTitle", nil, [CLImageEditorTheme bundle], @"Hair Style", @"");
}
...
{
    return [[[CLImageEditorTheme bundle] bundlePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@/Hairstyle", NSStringFromClass(self)]];
}

+ (NSDictionary*)optionalInfo
{
    return @{kCLHairstyleToolEmoticonPathKey:[self defaultEmoticonPath]};
}
...
{
    [_CLHairstyleView setActiveEmoticonView:nil];
...
- (void)setEmoticonMenu
{
    ...
    NSString *EmoticonPath = self.toolInfo.optionalInfo[kCLHairstyleToolEmoticonPathKey];
...
- (void)tappedEmoticonPanel:(UITapGestureRecognizer*)sender
{
    ...
    if(filePath)
    {
        _CLHairstyleView *view = [[_CLHairstyleView alloc] initWithImage:[UIImage imageWithContentsOfFile:filePath]];
        ...
        [_CLHairstyleView setActiveEmoticonView:view];
    }
...
@implementation _CLHairstyleView
{
...
+ (void)setActiveEmoticonView:(_CLHairstyleView*)view
{
    static _CLHairstyleView *activeView = nil;
    if(view != activeView)
    {
...
- (id)initWithImage:(UIImage *)image
{
    ...
    if(self){
        ...
        [_deleteButton setImage:[CLImageEditorTheme imageNamed:[CLHairstyleTool class] image:@"btn_delete.png"] forState:UIControlStateNormal];
...
{
    _CLHairstyleView *nextTarget = nil;
    
    const NSInteger index = [self.superview.subviews indexOfObject:self];
    
    for(NSInteger i=index+1; i<self.superview.subviews.count; ++i){
        UIView *view = [self.superview.subviews objectAtIndex:i];
        if([view isKindOfClass:[_CLHairstyleView class]]){
            nextTarget = (_CLHairstyleView*)view;
            break;
        }
    }
